...
extern var position_in: @Vector(4, f32) addrspace(.input);
...
export fn main() callconv(.spirv_vertex) void {
    ...
    std.gpu.location(&color_in, 2);

    std.gpu.location(&tex_coord_out, 0);
    std.gpu.location(&color_out, 1);

    std.gpu.position_out.* = uniforms.transform.mulVec(position_in);
